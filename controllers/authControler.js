...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(new AppError('You are not logged in , plese log in', 401));
  }
  ...
  // 4 Check if user changed password after Token was issiued
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  await user.save();
  //3 Updat ChangePasswordAt property
  createSendToken(user, 200, res);
  // const token = signToken(user._id);
  // res.status(200).json({
  //   status: 'success',
